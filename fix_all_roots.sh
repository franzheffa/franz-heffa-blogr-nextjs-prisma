...
for R in "${ROOTS[@]}"; do
  [ -d "$R" ] || continue
  echo "→ Patching root: $R"

  mkdir -p "$R/api" "$R/public"

  # Supprime tout ancien handler TS/JS qui pourrait entrer en conflit
  rm -f "$R/api/agent.ts" "$R/api/agent.js" "$R/api/agent.mjs" || true

  # Handler CommonJS (compatible partout, même avec "type":"module")
...
HTML
  ...
  # vercel.json dans chaque racine candidate (expansion de $GATEWAY voulue ici)
...
# Forcer l’add (même si .gitignore)
ADD_LIST=()
for R in "${ROOTS[@]}"; do
  [ -d "$R" ] || continue
  ADD_LIST+=("$R/vercel.json" "$R/api/agent.cjs" "$R/public/index.html")
done

git add -f "${ADD_LIST[@]}"
git commit -m "fix: add API function + vercel.json + test page in all candidate roots" || true
git fetch origin
